...
public class GoodsController {
    ...
    public Msg getAllGoods(@RequestParam(value = "page", defaultValue = "1") Integer pn, HttpServletResponse response, Model model, HttpSession session) {
        ...
        List<Goods> goods;
        String obj = (String) redisUtil.get("goods");

        if (obj == null){
            goods = goodsService.selectByExample(new GoodsExample());
            if (goods != null && goods.size() > 0){
                redisUtil.set("goods", JSONObject.toJSONString(goods));
            }
        }else {
            goods = JSON.parseArray(obj, Goods.class);
        }
    ...
    public String addGoods(Goods goods,
        ...
        goods.setActivityid(2);
        List list = new ArrayList();
        list.add(goods.getPrice());
        list.add(goods.getGoodsname());
        list.add(goods.getUptime());
        list.add(goods.getDetailcate());
        list.add(goods.getNum());
        list.add(goods.getActivityid());
        list.add(goods.getCategory());
        list.add(goods.getDescription());
...
//                System.out.println(realPath);
                ...
                String imagePath = realPath.substring(0,realPath.indexOf("shop")) +"\\shop\\src\\main\\webapp\\shopimage\\" + imageName;
